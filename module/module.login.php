...
function head_login(){
	if(isset($_POST['login'])){
		...
		if(empty($username_error) & empty($password_error)){
			$valid = mysql_fetch_array(mysql_query("SELECT id FROM staff 
			WHERE username='".$_POST['username']."' 
			AND password=MD5('".$_POST['password']."') AND status='admin'"));
			if(!empty($valid[id])){
				mysql_query("INSERT INTO login VALUES(null,'".$valid['id']."',null)") 
				or die(mysql_error());
...
function main_login_list(){
	global $date_format;
	$title = 'Login List';
	$head = array('Username','From Date');
	$width = array('50%','50%');
	$content1 = 'index.php?p=login_list&none=';
	$content2 = 'index.php?p=login_list&delete=';
	$q = mysql_query("SELECT *,".$date_format." FROM login ORDER BY id DESC");
	$rows = array();
	$ids = array();
	while($data = mysql_fetch_array($q)){
		$username = mysql_fetch_row(mysql_query("SELECT username FROM admin WHERE id='".$data[admin_id]."'"));
		$d = array($username[0],$data[datef]);
		array_push($rows,$d);
